...
QUnit.test("findByOutput() Dres Lantern, Stationary", (assert) => {
  ...
  assert.equal(ratio >= 1.908 && ratio <= 2.1042, true, `ratio = ${ratio}`);
...
QUnit.test("inputValue()", (assert) => {
  ...
  // Verify.
  assert.equal(result >= 366.94 && result <= 496, true, `result = ${result}`);
});
...
QUnit.test("outputInputRatio()", (assert) => {
  ...
  // Verify.
  assert.equal(result >= 1.908 && result <= 2.1042, true, `result = ${result}`);
});
